...
const PROMPT =
	process.env.PI_WEB_SUBAGENT_PROMPT ??
	"Call the subagent tool with task 'say hello in one word'. Then say exactly: subagent done.";
...
async function main() {
	...
	while (Date.now() < deadline) {
		...
		subagentOk =
			Array.isArray(toolTexts) && toolTexts.some((text) => text.includes("say hello in one word"));
		assistantOk =
			Array.isArray(assistantTexts) &&
			assistantTexts.some((text) => text.toLowerCase().includes("subagent done"));
		if (subagentOk && assistantOk) break;
		await sleep(1000);
	}

	const ok = subagentOk && assistantOk;
	socket.close();
	console.log(JSON.stringify({ assistantOk, assistantTexts, ok, subagentOk, toolTexts }, null, 2));
	process.exit(ok ? 0 : 1);
}
