...
const PROMPT =
	process.env.PI_WEB_ASK_PROMPT ??
	"Call the ask_user_test tool with question 'What is 2+2?', then say exactly: answered.";
const ANSWER = process.env.PI_WEB_ASK_ANSWER ?? "4";
...
async function main() {
	...
	if (!dialogSeen) throw new Error("ask_user dialog did not appear");
	...
	await evaluate(`(() => {
		...
		const submit = buttons.find((button) => button.textContent?.trim() === "Submit");
		submit?.click();
		return true;
	})()`);
	...
	while (Date.now() < resultDeadline) {
		...
		assistantText = Array.isArray(texts)
			? (texts.find((text) => text.toLowerCase().includes("answered")) ?? texts.at(-1) ?? null)
			: null;
		if (typeof assistantText === "string" && assistantText.toLowerCase().includes("answered")) break;
		await sleep(500);
	}

	const ok = typeof assistantText === "string" && assistantText.toLowerCase().includes("answered");
	const bodyText = await evaluate(`document.body.innerText`);
	const messages = await evaluate(`window.pi.getMessages()`);
	socket.close();
	console.log(JSON.stringify({ assistantText, bodyText, dialogSeen, messages, ok }, null, 2));
	process.exit(ok ? 0 : 1);
}
