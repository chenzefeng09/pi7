...
export function PluginsSection() {
	...
	return (
		<div className="flex flex-col">
			<GroupHeading>{t("Goal 插件包")}</GroupHeading>
			<div className="rounded-xl border-[0.5px] border-black/[0.08] px-5">
				<SettingsRow
					description={t("为 pi 提供目标跟踪和任务完成能力，来自 npm:pi-goal-x。两个 Goal 插件包都注册 /goal，一次只能安装一个。")}
					title={t("目标跟踪（pi-goal-x）")}
				>
					<PluginInstallButton
						busy={busy}
						installed={installed.has("npm:pi-goal-x")}
						onClick={() => void run(() => installPackage("npm:pi-goal-x", local), t("正在安装 pi-goal-x…"))}
					/>
				</SettingsRow>
				<SettingsRow
					description={t("Narumitw 提供的 Goal 扩展，来自 npm:@narumitw/pi-goal。")}
					title={t("目标跟踪（@narumitw/pi-goal）")}
				>
					<PluginInstallButton
						busy={busy}
						installed={installed.has("npm:@narumitw/pi-goal")}
						onClick={() =>
							void run(() => installPackage("npm:@narumitw/pi-goal", local), t("正在安装 @narumitw/pi-goal…"))
						}
					/>
				</SettingsRow>
			</div>

			<GroupHeading>{t("安装")}</GroupHeading>
			<div className="flex flex-col gap-2">
				<div className="flex gap-2">
					<input
						className={`${SETTINGS_INPUT_CLASS} min-w-0 flex-1 font-mono`}
						onChange={(event) => setSource(event.target.value)}
						placeholder={t("npm:@scope/pkg、git:github.com/user/repo、./local/path")}
						value={source}
					/>
					<button
						className={buttonClass("primary", "sm")}
						disabled={busy || !source.trim()}
						onClick={() => void run(() => installPackage(source.trim(), local), t("正在安装…"))}
						type="button"
					>
						{t("安装")}</button>
				</div>
				<div className="flex items-center gap-2">
					<span className="text-[12px] text-[#98a2b3]">{t("作用域")}</span>
					<SettingsSelect
						align="start"
						minWidth={96}
						...
						options={[
							...
							{ label: t("当前项目"), value: "project" },
						]}
						title={t("选择安装作用域")}
						value={local ? "project" : "global"}
					/>
					<span className="text-[11px] text-[#98a2b3]">{t("项目作用域写入该项目的 .pi/settings.json")}</span>
				</div>
			</div>
			{message ? <div className="pt-2 text-[11px] text-[#667085]">{message}</div> : null}

			<GroupHeading>{t("已安装")}</GroupHeading>
			<div className="rounded-xl border-[0.5px] border-black/[0.08] px-5">
				{packages.global.map((item, index) => (
					<SettingsRow
						description={t("对本机所有项目生效")}
					...
					>
						<PluginRemoveButton
							...
							onClick={() => void run(() => removePackage(sourceText(item), false), t("正在移除…"))}
						/>
					</SettingsRow>
				))}
				{packages.project.map((item, index) => (
					<SettingsRow
						description={t("只对当前项目生效")}
					...
					>
						<PluginRemoveButton
							busy={busy}
							label={sourceText(item)}
							onClick={() => void run(() => removePackage(sourceText(item), true), t("正在移除…"))}
						/>
					</SettingsRow>
				))}
				{packages.global.length === 0 && packages.project.length === 0 ? (
					<SettingsRow
						description={t("从上面的「安装」加一个插件包，或装一个 Goal 插件包")}
						title={t("没有已安装的插件包")}
					/>
				) : null}
				<SettingsRow description={t("按 pi 的版本重新解析每个已安装插件包")} title={t("更新已安装插件包")}>
					<button
						className={buttonClass("secondary", "sm")}
						disabled={busy}
						onClick={() => void run(() => updatePackages(), t("正在更新…"))}
						type="button"
					>
						<RotateCw size={12} />
						{t("更新")}</button>
			...
			<div className="flex items-center gap-1 pt-2 text-[11px] text-[#98a2b3]">
				<Puzzle size={12} />
				{t("插件包会在 pi 进程里执行本地代码，安装前请确认来源。")}</div>
		</div>
	);
}
