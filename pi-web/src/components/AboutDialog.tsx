...
const SOURCE_LABELS: Record<string, string> = {
	bundled: "随应用打包的 config/agent",
	env: "环境变量 PI_CODING_AGENT_DIR",
	home: "用户目录默认 ~/.pi/agent",
	portable: "便携目录 data\\agent",
};
...
export function AboutDialog({ onClose, open }: { onClose: () => void; open: boolean }) {
	...
	return (
		...
		>
			...
			<div className="mt-1 text-[13px] text-[#667085]">Pi Agent 的 Win7 兼容版</div>
			<div className="mt-1 text-[12px] text-[#98a2b3]">版本 {info?.appVersion ?? "0.1.0"}</div>
			...
			<div className="mt-5 space-y-1.5 rounded-xl border border-black/[0.06] bg-[#fafbfa] p-3 text-left">
				...
				/>
				<Row label="Node" mono value={info?.nodePath ?? "未连接"} />
				<Row label="Pi 运行时" mono value={info?.cliPath ?? "未连接"} />
				<Row
					hint={SOURCE_LABELS[info?.agentDirSource ?? ""] ?? info?.agentDirSource}
					label="配置目录"
					mono
					value={info?.agentDir ?? "未连接"}
				/>
