...
export function ExcelPreview({ blob }: { blob: Blob }): JSX.Element {
	...
	useEffect(() => {
		...
		void blob
			...
			.catch((reason: unknown) => {
				if (!cancelled) setError(reason instanceof Error ? reason.message : t("无法解析表格"));
			});
		return () => {
			cancelled = true;
		};
	...
	if (error) return <div className="p-4 text-[12px] text-[#f04438]">{t("表格解析失败：")}{error}</div>;
	if (!sheets) return <div className="p-4 text-[12px] text-[#98a2b3]">{t("解析表格...")}</div>;
	...
	return (
		<section className="overflow-hidden rounded-lg border border-black/[0.06] bg-white">
			<div className="flex flex-wrap items-center gap-2 border-b border-black/[0.06] px-4 py-2">
				<span className="text-[12px] font-medium text-[#667085]">{t("表格预览")}</span>
			...
			<div className="max-h-[calc(100vh-170px)] overflow-auto">
				{sheet && sheet.rows.length > 0 ? (
					...
					<div className="p-4 text-[12px] text-[#98a2b3]">{t("（空工作表）")}</div>
				)}
				{sheet && sheet.rows.length > 1000 ? <div className="p-3 text-[11px] text-[#98a2b3]">{t("仅显示前 1000 行")}</div> : null}
			</div>
		</section>
	);
}
